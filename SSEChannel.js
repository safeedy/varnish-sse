...
module.exports = class SSEChannel {
	...
	constructor(options) {
        ...
        this.messages = [];
        this.prePublishLength = 0;
        this.postPublishLength = 0;
    ...
    resetMetrics() {
        this.prePublishLength = this.postPublishLength = 0;
        this.nextID = 1;
    }
	...
	publish(data, eventName) {
        ...
        const length = Buffer.byteLength(output);
        this.prePublishLength = this.postPublishLength;

        [...this.clients].filter(c => !eventName || hasEventMatch(c.events, eventName)).forEach(c => {
            /* c.res.cookie('X-Event-Sent-Id', `${id}`, 1000);
            c.res.cookie('X-Event-Sent-Offset', `${this.prePublishLength}`, 1000); */
            c.res.write(output);
        });
        
        this.postPublishLength += length; 
